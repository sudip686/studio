...
(()=>{
  ...
  const panels=selectors.flatMap(selector=>[...document.querySelectorAll(selector)].filter(visible).map(el=>({selector,rect:el.getBoundingClientRect().toJSON(),scrollable:el.scrollHeight>el.clientHeight+2})));
  const overlaps=[];
